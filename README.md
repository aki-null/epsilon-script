...
EpsilonScript is a lightweight, simple, dynamically typed expression language designed for embedded use in games written in C#.
...
- Simple syntax without too much flexibility
- Supports Unity
- No heap allocation after compilation (with exceptions)

## Project State
Mostly usable in production. There are not enough tests for some components of the software, which is being worked on now.
...
Unique integers generated by `GetUniqueIdentifier` extension method are used to address variable names for performance reasons. The resolved identifier integer should be cached for later use.
...
It doesn't use Expression Trees because runtime IL generation doesn't work on ahead-of-time (AOT) compiled environments like Unity's IL2CPP.